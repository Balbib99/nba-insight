  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <PageHero
        eyebrow="Players"
        title="NBA Players"
        description="Explore a curated mock player dataset through the same service layer that can later connect to a real NBA API."
        tags={[isLoading ? 'Loading players' : `${filteredPlayers.length} of ${players.length} players`]}
      />
      <div className="mt-4">
      ...
      <div className="mt-8 border-t border-rule bg-ink-900 p-4">
        <div className="grid gap-4 lg:grid-cols-[1fr_210px_240px_auto] lg:items-end">
          ...
          />
          <SelectFilter id="team-filter" label="Team" options={teamOptions} value={teamId} onChange={setTeamId} />
          <button
      ...
      {error ? (
        <div className="mt-8 border border-down/30 bg-down/10 p-5 text-sm text-text-primary">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-display text-base font-semibold text-text-primary">Unable to load players</h2>
      ...
      {!isLoading && !error && filteredPlayers.length === 0 ? (
        <div className="mt-8 border border-rule bg-ink-900 p-8 text-center">
          <h2 className="font-display text-xl font-semibold text-text-primary">No players found</h2>
